...
public class BlockMoneyLog extends BlockRotatedPillar {

	public String unlocName = BlockInfo.MONEY_LOG_NAME_UNLOC;
	
	public BlockMoneyLog(int id, Material material) {
		super(id, material);
    ...
    public static final String[] woodType = new String[] {"Money"};
     ...
     */
    protected Icon getEndIcon(int side)
    {
        return this.tree_top[side];
    }
    ...
    {
        this.icons = new Icon[woodType.length];
        this.tree_top = new Icon[woodType.length];

        for (int i = 0; i < this.icons.length; ++i)
        {
            this.icons[i] = par1IconRegister.registerIcon(ModInfo.ID + ":" + "log" + woodType[i] + "_Side");
            this.tree_top[i] = par1IconRegister.registerIcon(ModInfo.ID + ":" + "log" + woodType[i] + "_Top");
        }
    }
